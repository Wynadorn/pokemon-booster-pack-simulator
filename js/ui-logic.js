...
function removeDuplicates(allCards) {
    ...
    function getUniqueCardsFrom(cardArr) {
        const uniques = [];
        let uniqueNames = [...new Set(cardArr.map((card) => card.id))]; // Extract unique names by putting array of all names into a set
        uniqueNames.forEach((id) => {
            const firstMatchingCard = cardArr.find((card) => card.id === id); // Then use the names array to find a single match for each name
